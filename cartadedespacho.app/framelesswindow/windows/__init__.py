...
class WindowsFramelessDialog(WindowsFramelessWindowBase, QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent=parent)
        self._initFrameless()
        # Keep min/max buttons — dialogs with tables need maximize
        # Do NOT call disableMaximizeButton — it corrupts parent window state

    def closeEvent(self, event):
        super().closeEvent(event)
        try:
            parent_win = self.parent().window() if self.parent() else None
            if parent_win and hasattr(parent_win, 'updateFrameless'):
                from PySide6.QtCore import QTimer
                def _do_restore():
                    parent_win.updateFrameless()
