...
def generateCesiones(formData, endorsementData, savePath):
    """
    ...
    try:
        ...
        def _add_table_3col(branch, numero, rows, currency):
            """Add SectionStart, Heading, and a 3-column table."""
            sec = _new_para()
            _set_style(sec, 'SectionStart')
            _noto_run(sec, '\u00A0', size_pt=36)
            ...
            hdg = _new_para()
            _set_style(hdg, 'Heading')
            label = f"PÓLIZA {branch.upper()}"
            if numero:
                label += f" {numero}"
            _noto_run(hdg, label, size_pt=18, bold=True)
            _zero_spacing(hdg)
            ...
            # Build table via python-docx API (generates valid w:tblGrid)
            # then move it before body_sectPr
            tbl = doc.add_table(rows=1 + len(rows), cols=3)
            tbl._element.getparent().remove(tbl._element)
            body.insert(list(body).index(body_sectPr), tbl._element)

            headers = ['Detalle', f'Monto Endosado {currency}', 'Endosatario']
            hrow = tbl.rows[0]
            ...
            for ri, row in enumerate(rows):
                dr = tbl.rows[ri + 1]
                dr.cells[0].text = row.get('detalle','')
                dr.cells[1].text = row.get('monto','')
                dr.cells[2].text = row.get('endosatario','')
                for cell in dr.cells:
                    for run in cell.paragraphs[0].runs:
                        run.font.name = 'Noto Sans'
                        run.font.size = Pt(10)

        def _add_table_trec(branch, numero, groups, currency):
            """Add SectionStart, Heading, and per-group 8-column TREC tables."""
            sec = _new_para()
            _set_style(sec, 'SectionStart')
            _noto_run(sec, '\u00A0', size_pt=36)
        ...
        for key, entry in endorsementData.items():
            ...
            # Resolve branch and numero from key (insuredId::policyId or similar)
            # These are passed in entry if collected from widget metadata
            branch = entry.get('branch', '')
            numero = entry.get('numero', '')
            is_trec = branch.upper() == 'TREC'

            if is_trec:
                groups = entry.get('groups', [])
                _add_table_trec(branch, numero, groups, currency)
            else:
                rows = entry.get('rows', [])
                _add_table_3col(branch, numero, rows, currency)
