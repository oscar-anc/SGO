...
class CustomTitleBar(TitleBar):
    ...
    def __init__(self, parent):
        ...
        from theme import QSSA as _T
        btn_text     = QColor(_T.get('titlebar_btn_text',        '#FFFFFF'))
        btn_hover    = QColor(_T.get('titlebar_btn_hover_bg',    '#59c2e6'))
        btn_pressed  = QColor(_T.get('titlebar_btn_pressed_bg',  '#3aaac8'))
        close_hover  = QColor(_T.get('titlebar_close_hover_bg',  '#fc4c7a'))
        close_pressed= QColor(_T.get('titlebar_close_pressed_bg','#d93060'))
...
def main():
    ...
    # Load Open Sans from fonts/ folder so it is always available
    # regardless of whether it is installed on the system.
    # Download from fonts.google.com/specimen/Open+Sans and place in fonts/
    for fontFile in ["fonts/OpenSans-Regular.ttf", "fonts/OpenSans-Bold.ttf",
                     "fonts/OpenSans-SemiBold.ttf"]:
        QFontDatabase.addApplicationFont(resource_path(fontFile))
